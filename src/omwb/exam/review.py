...
def build_review_prompt(exam: dict, code: str, lang: str, code_path: str) -> list[dict]:
    """组装审查 prompt:题目 + 带行号代码 + 批注 JSON schema。"""
    numbered = "\n".join(f"{i + 1} | {line}" for i, line in enumerate(code.splitlines()))
    exam_part = json.dumps(
        {k: exam.get(k) for k in ("title", "task", "output_spec", "constraints", "hints", "concepts")},
    ...
    user = (
        ...
        '"annotations": [{"segment": str 对应片段, "text": str 为什么这么改, "concrete_explanation": str 具象化解释}]}}。\n'
...
def review_code(exam_path: str | Path, code_path: str | Path, out: str | Path = "omwb-out",
    ...
    exam_id = exam.get("id") or Path(exam_path).stem
    out_dir = Path(out) / site / "exam"
    out_dir.mkdir(parents=True, exist_ok=True)
    base = out_dir / f"{exam_id}-review"
    base.with_suffix(".md").write_text(render_markdown(exam, report, lang), encoding="utf-8")
